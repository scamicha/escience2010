...
The rest of this paper is arranged as follows: \S\ref{sec:history} contains a brief history of digital imaging in astronomy and the reduction and analysis of those images. In \S\ref{sec:current} we describe the current paradigm employed by astronomers for dealing with imaging data and outline some upcoming problems with this paradigm. We detail a new paradigm for the processing of astronomical images in \S\ref{sec:rev}. In \S\ref{sec:ODI} we describe how this new paradigm can apply to the ODI instrument. Finally, in \S\ref{sec:future} we explore possible future directions and in \S\ref{sec:conclusions} we present our final conclusions.

\section{A Brief History of Digital Image Processing in Astronomy}\label{sec:history}
...
The design of the ODI data reduction pipeline and archive system can be though of as having five major hardware and software components. These are the network used to transfer data from the instrument and to compute facilities, the compute facilities that are used to do the actual reduction and analysis, the temporary and long term storage used to hold data for analysis and permanently store raw data, the software orchestration layer that distributes compute jobs and input data to the various compute facilities, and the software reduction and analysis software, or pipeline engine, that contains the actual astronomical algorithms to be used to analyze the data.   
...
The Data Capacitor will host 200TB of short to mid-term storage for the analysis and production of Tier 1 and Tier 2 data products. Users with observing time will be granted quotas based on the projected amount of data that will be collected during their observations. An initial estimate for the quota is three times the projected size of the raw data. Assuming that a single program will obtain 5--10TB of data, the 200TB of Data Capacitor storage will allow for the processing of data from 6--12 projects simultaneously. As demand for Data Capacitor service grows, the amount of space available on the Data Capacitor will increase as well. We project that the initial 200TB will be sufficient for the first 5 years of development and operation. In addition to quotas being granted to users with observing time, some amount of space will be made available to users interested in using archival data only. Initially this will be a small amount of space, but it will increase with the size of the archive. To prevent overruns on the Data Capacitor space, files that have been pulled from the archive and remain unused for some time will be deleted. However, the user will be able to quickly and easily restore them if they are needed again.

The MDSS will host the ODI archive, which will store all raw science images and calibration data. The data will actually reside in two locations -- in Bloomington and on a mirrored system in Indianapolis. Any failure at one site can be recovered from by the other. The capacity of the MDSS is more than sufficient to store the volume of data expected from ODI, and it is a proven system that provides fast and reliable retrieval. 
Access to data in the archive will be restricted to the PI, and any users that he or she specifies, during the 18 month proprietary period following the observation. After this time, the raw (and Tier 1) data will become public. The PMD will contain searchable metadata entries for the Tier 1 data products, as well as the metadata for the Tier 1 processing itself (the workflow). The latter will allow the reproduction of Tier 1 data from the raw archived data. 

Data transfer from the WIYN telescope to IU begins once the observing team has gathered an observation, the data and metadata files are available, and the transfer to the Data Capacitor has been queued in software running on the local buffer on the mountain. 
This buffer will provide two weeks of raw data storage to smooth the data flow and mitigate interruptions due to network outages and system maintenance.
IU will maintain a system that has read access to the summit local buffer and also mounts the DC. As detailed in \S\ref{sec:accomplish} we have established a stable Data Capacitor mount from Tucson to IU which can meet the required data transfer rate. (However, The DC has never been mounted across a microwave link, so the performance that will be achieved after the upgrade of the link between the summit and Tucson is unknown.)  A checksum of all of the files transferred to the Data Capacitor is computed at WIYN and sent with the data. Once the files have arrived, a checksum is computed for the files on the Data Capacitor. Upon successful checksum match, the Data Capacitor sends a message back to the transfer software and the status of the transfer is changed to "pending confirmation." If the checksum match is unsuccessful, the file transfer is re-queued and attempted again. After some number of unsuccessful transfers, both the WIYN Pipeline team and Data Capacitor support team would be notified.

Next, the metadata file is ingested into the PMD. This triggers the copying of files from the Data Capacitor to the MDSS (and the MDSS mirror site). Once this transfer is completed, a checksum is generated for the files on MDSS which is compared to the Data Capacitor checksum. On successful checksum comparison, the PMD communicates to the WIYN local buffer that the files have been replicated. They are then deleted from the local buffer and the transfer is removed from the queue. If the checksum comparison fails, the transfer from the Data Capacitor to MDSS is re-initiated. Raw files which reside on MDSS will be compressed; all compression will be done in a lossless fashion.

New images are queued for Tier 1 processing by the workflow system following the ingestion of the metadata and the verification of the files on the DC and MDSS. 
Based on the metadata associated with the image (e.g., observing mode, filter, moon phase, etc.), appropriate master calibrations and processing parameters are selected. The commands to be issued to OIPP are constructed based on these selections, and the TeraGrid resource best suited for the job is determined. The workflow system then takes the input data, calibration data, and OIPP commands and generates the appropriate batch submission script for the particular TeraGrid resource that will run the job. TeraGrid resources with Data Capacitor access will be included in job scheduling and the input data will already be available on the supercomputer. The job is executed on the TeraGrid resource and the results are written back to the Data Capacitor. The PMD is then populated with information about the job, as well as statistics of the result data. If the input is calibration data, these statistics are used to generate appropriate quality assurance metrics. 

After the processing job has completed and results are available, the ODI pipeline scientist is notified and he or she can then check the metrics and the data products to ensure that they meet quality standards. The pipeline scientist can confirm the Tier 1 processing or re-queue it to run again, possibly with different processing parameters, depending on the outcome of the assessment. Once the Tier 1 processing has been confirmed by the pipeline scientist, the PI is notified. The PI can then log into the science gateway to view his or her Tier 1 processed data and begin Tier 2 processing or initiate a download.

Tier 1 data products will be stored on the Data Capacitor for several weeks after being confirmed by the pipeline scientist. At the end of this period, or if they have not been accessed by the PI at all for a few weeks, they will be migrated off of the DC. The processing metadata for the confirmed Tier 1 run will be saved so that the Tier 1 data products can be re-created using the same workflow, i.e., the same processing steps, parameters, and calibration data. (However, data products will be re-created using the most current versions of the pipeline software, which should not change frequently.) This re-creation will occur seamlessly as needed, without the need for user interaction. The only indication of the reprocessing would be a delay in the initiation of a Tier 2 workflow or Tier 1 download.

   % any refs to this deleted section ??  \subsection{User Driven Tier 2 Processing} \label{sec:tier2}
The part of this proposed solution most visible to the astronomical community will be the science gateway. Science gateways provide a way for entire communities of users to access shared resources through a customized web-accessible interface. Since a science gateway is much more complex than an average website, it does more than simply provide information and links to other websites. Science gateways typically provide access to user data and data collections, community software, workflows, job execution, visualization, and social networking. They are especially useful in increasing productivity by shielding end users from the complexities of accessing data and running jobs on high-end compute resources with varying access mechanism.
...
The ODI gateway that we propose to create will be very similar in many ways to the LEAD Science Gateway (Linked Environments for Atmospheric Discovery), a TeraGrid Science Gateway developed at IU. LEAD is used by students and researchers for meteorological analysis and modeling. It was developed over 6 years as a research project, for a cost of \$11 million (which does not include the costs of the preceding basic research into grid portal technology). By building on this past work, co-opting, customizing, and modifying basic LEAD components, and adding our own specialized modules, we can obviously achieve substantial time and cost savings, while also building a unique and highly productive gateway optimized for ODI.
